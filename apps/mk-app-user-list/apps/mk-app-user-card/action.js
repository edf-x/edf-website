...
class action {
    ...
    load = async () => {
       ...
       // payload.departments = response
        ...
        let roleIds = response.roles && response.roles.map(o => o.roleId)
    ...
    save = async () => {
        ...
        let roleIds = this.metaAction.gf('data.other.selectedRoles') || []
        let roles = roleIds.map(oid => {
            let mo = { userId: form.id, roleId: oid }
            if (form.roles) {
                let role = form.roles.find(m => m.roleId == oid)
                if (role) {
                    mo.id = role.id
                }
            }
        ...
        if (form.id || form.id == 0) {
            ...
            if (response) {
                this.metaAction.toast('success', '用户修改成功')
            ...
            if (response) {
                this.metaAction.toast('success', '用户保存成功')
                this.injections.reduce('setPerson', response)
            }
        }
        return true
    ...
    loadRoles = async () => {
        const roleLists = await this.webapi.role.query({})
        let roles = roleLists.map(o => ({ label: o.name, value: o.id }))
        this.injections.reduce('loadRoles', { roleLists, roles })
    }

    selectRoles = (curIds) => {
        this.injections.reduce('selectRoles', curIds)
    }
}
