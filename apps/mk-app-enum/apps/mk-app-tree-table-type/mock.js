...
fetch.mock('/v1/enum/create', (option) => {
    const parent = findNode(option.parentId, mockData.enumTypes)
    var newId = -1
    if (!parent.children) {
        parent.children = []
...
fetch.mock('/v1/enum/update', (option) => {
    const node = findNode(option.id, mockData.enumTypes)
    node.code = option.code
    node.name = option.name
    return { result: true, value: node }
})

fetch.mock('/v1/enum/delete', (option) => {
    const node = findNode(option.id, mockData.enumTypes)
    node.code = option.code
    node.name = option.name
    return { result: true, value: node }
...
fetch.mock('/v1/enum/findById', (id) => {
    const node = findNode(id, mockData.enumTypes)
    return { result: true, value: node }
})