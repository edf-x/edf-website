...
class action {
        ...
        operations.forEach(op => {
            let col = <Column name={op.id} columnKey={op.id} flexGrow={1} width={200}
                ...
                cell={(ps) => {
                    ...
                    return <Cell><Checkbox disabled={!roleId} checked={checked} onChange={this.selectOperation(m, op)} >{op.name}</Checkbox></Cell>
                }}
            />
            cols.push(col)
        })
    ...
    selectRole = async ({ selectedKeys }) => {
        ...
        const menuOperations = await this.webapi.menuOperation.query(filter)
        this.injections.reduce('loadRoleMenuOperation', { menuOperations, filter })
    }
    ...
    selectAll = (operation) => (e) => {
        ...
        this.webapi.menuOperation.save(menuOperations)
    ...
    addRole = async () => {
        const ret = await this.metaAction.modal('show', {
            title: '新增',
            children: this.metaAction.loadApp('mk-app-role', {
