...
let myServiceDefine = ({ menu, menuOperation, operation }) => 0 || {
    menu: {
        create: async (data, ctx) => {
            ...
            let moIds = await menuOperation.executeSql('select max(id)+1 id  from menuOperation')
            ...
            if (data.operations && data.operations.length != 0) {
                ...
                await menuOperation.createBatch(data.operations)
            }
            return data
        },
        findFullById: (id, ctx) => {
...
webProvider && webProvider({
    '/v1/menu/create': service.menu.create,
    '/v1/menu/update': service.menu.update,
    '/v1/menu/findFullById': service.menu.findFullById
})
